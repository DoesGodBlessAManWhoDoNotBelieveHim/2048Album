...
- (void)setAssetsGroup:(ALAssetsGroup *)theAssetsGroup
{
    @synchronized (self)
    {
        if (_assetsGroup != theAssetsGroup)
        {
            ...
            if (_imagePickerController.pickerType == PickerTypePhotos) {
...
- (void)changeSelectionInformation
{
    if (self.imagePickerController.shouldDisplaySelectionInformation ) {
        if (0 == [AGIPCGridItem numberOfSelections] ) {
            ...
            if (0 < maxNumber) {
                self.navigationController.navigationBar.topItem.prompt = [NSString stringWithFormat:@"(%d/%d)", (int)[AGIPCGridItem numberOfSelections], (int)maxNumber];
            } else {
                self.navigationController.navigationBar.topItem.prompt = [NSString stringWithFormat:@"(%d/%d)", (int)[AGIPCGridItem numberOfSelections], (int)self.assets.count];
            }
        }
    }
}
